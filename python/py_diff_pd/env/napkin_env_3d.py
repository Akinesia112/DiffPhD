...
class NapkinEnv3d(EnvBase):
    ...
    def __init__(self, seed, folder, options):
        ...
        het_flag = options['het'] if 'het' in options else False
        ...
        # Half stiff / half soft split by element index order.
        n = cell_nums[0] * cell_nums[1] * cell_nums[2]
        stiff_ele = list(range(n // 2))
        soft_ele  = list(range(n // 2, n))

        if not het_flag:
            deformable.AddPdEnergy('corotated', [2 * mu,], [])
            deformable.AddPdEnergy('volume', [la,], [])
        else:
            print("len of stiff elements: ", len(stiff_ele), max(stiff_ele))
            print("len of soft elements: ", len(soft_ele), max(soft_ele))
            deformable.AddPdEnergy('corotated', [2 * mu,], stiff_ele)
            deformable.AddPdEnergy('volume', [la,], stiff_ele)
            deformable.AddPdEnergy('corotated', [2 * mu * 0.1,], soft_ele)
            deformable.AddPdEnergy('volume', [la * 0.1,], soft_ele)
    ...
    def material_stiffness_differential(self, youngs_modulus, poissons_ratio):
        # Rows must match the order of AddPdEnergy calls so that
        # dl_dmat_w (shape [num_pd_energies]) @ jac aligns with (E, nu).
        jac = self._material_jacobian(youngs_modulus, poissons_ratio)
        if not self.__het_flag:
            jac_total = np.zeros((2, 2))
            jac_total[0] = 2 * jac[1]   # d(2*mu)/d(E, nu)
            jac_total[1] =     jac[0]   # d(la)/d(E, nu)
            return jac_total
        # Heterogeneous: 4 PD energies in this order:
        #   [stiff corotated, stiff volume, soft corotated, soft volume]
        # Soft weights are 0.1x the stiff ones.
        jac_total = np.zeros((4, 2))
        jac_total[0] = 2 * jac[1]
        jac_total[1] =     jac[0]
        jac_total[2] = 0.1 * 2 * jac[1]
        jac_total[3] = 0.1 *     jac[0]
        return jac_total
