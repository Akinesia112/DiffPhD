...
class RoutingTendonEnv3d(EnvBase):
    def __init__(self, seed, folder, options):
        ...
        if not het_bool:
            ...
            # deformable.AddPdEnergy('pd_neohookean', [2 * mu, la], [])
        else:
            stiff_ele = []
            soft_ele = []
            # upper half is softer, lower half is stiffer.
            for i in range(cell_nums[0]):
                for j in range(cell_nums[1]):
                    for k in range(cell_nums[2]):
                        ele_idx = i * cell_nums[1] * cell_nums[2] + j * cell_nums[2] + k
                        if i < cell_nums[0] // 2:
                            stiff_ele.append(ele_idx)
                        else:
                            soft_ele.append(ele_idx)
            print('Stiff element num: {}, soft element num: {}'.format(len(stiff_ele), len(soft_ele)))
            deformable.AddPdEnergy('corotated', [2 * mu,], stiff_ele)
            deformable.AddPdEnergy('volume', [la,], stiff_ele)
            deformable.AddPdEnergy('corotated', [2 * mu * 0.5,], soft_ele)
            deformable.AddPdEnergy('volume', [la * 0.5,], soft_ele)
    ...
    def material_stiffness_differential(self, youngs_modulus, poissons_ratio):
        ...
        if not self._het:
            ...
            jac_total[0] = 2 * jac[1]
            jac_total[1] = jac[0]
            jac_total[2] = 2 * 0.5 * jac[1]
            jac_total[3] = 0.5 * jac[0]
        return jac_total
    ...
    def _display_mesh(self, mesh_file, file_name):
        ...
        if not self._het:
            ...
            renderer.add_hex_mesh(stiff_mesh, render_voxel_edge=True, color=(.7, .3, .5), transforms=[
                ('s', 0.4),
            ])
            renderer.add_hex_mesh(soft_mesh, render_voxel_edge=True, color=(.3, .7, .5), transforms=[
                ('s', 0.4),
            ])
