...
def _alg_phd_s_memory_bytes():
    """Bytes held by the persistent AlgPhd GPU S / S^T CSR buffers.
    Returns 0 when built without CUDA or when AlgPhd forward hasn't run."""
    try:
        from py_diff_pd.core.py_diff_pd_core import AlgPhdSMemoryBytes
        return int(AlgPhdSMemoryBytes())
    except Exception:
        return 0


def _gpu_vram_used_bytes():
    """(total - free) bytes on the current CUDA device via cudaMemGetInfo.
    Returns 0 if libcudart missing / no active context."""
    try:
        import ctypes
        cudart = None
...
class EnvBase:
    ...
    def simulate(self, dt, frame_num, method, opt, q0=None, v0=None, act=None, f_ext=None,
        require_grad=False, vis_folder=None, velocity_bound=np.inf, render_frame_skip=1, skip_first=0):
        # Check input parameters.
        assert dt > 0
        assert frame_num > 0
        if isinstance(method, str):
            if method.startswith('pd_eigen_cuda_mas_pcg'):
                forward_method = 'pd_eigen_pcg'
                backward_method = method
            elif method.startswith('pd_eigen_mas_pcg'):
                forward_method = 'pd_eigen_pcg'
                backward_method = method
            elif method.startswith('pd_eigen_pcg'):
                forward_method = 'pd_eigen_pcg'
            ...
            raise NotImplementedError
        # Keys consumed by Python only — must not be forwarded to C++ SWIG bindings.
        _PYTHON_ONLY_KEYS = {'backward_grad_clip_norm'}
        def _cpp_opt(d):
            if isinstance(d, dict):
                return {k: v for k, v in d.items() if k not in _PYTHON_ONLY_KEYS}
            return d

        if isinstance(opt, dict):
            forward_opt = _cpp_opt(opt)
            backward_opt = opt          # kept as-is for Python grad_clip_norm read
            backward_opt_cpp = _cpp_opt(opt)
        elif isinstance(opt, tuple):
            assert len(opt) == 2
            forward_opt = _cpp_opt(opt[0])
            backward_opt = opt[1]
            backward_opt_cpp = _cpp_opt(opt[1])
        ...
        for i in range(frame_num):
            # Record the time.
            # print(f"Simulating frame {i + 1}/{frame_num} with method {forward_method}...")
            t_begin = time.time()
        ...
        # GPU memory snapshot (AlgPhd S factor + whole-process VRAM). Best-effort
        # only — returns 0 on CPU-only builds or when AlgPhd hasn't run yet.
        info['s_memory_bytes']       = _alg_phd_s_memory_bytes()
        info['gpu_vram_used_bytes']  = _gpu_vram_used_bytes()
        ...
        if vis_folder is not None:
            ...
            for i, qi in enumerate(q):
                ...
                # qi_flat = ndarray(qi).ravel()
                # qi_finite = bool(np.isfinite(qi_flat).all())
                # if qi_flat.size % 3 == 0:
                #     qi_xyz = qi_flat.reshape(-1, 3)
                #     #print 0%, 25%, 50%, 75%, 100% quantiles of each dimension.
                #     quantiles = np.percentile(qi_xyz, [0, 25, 50, 75, 100], axis=0)
                #     print('[DEBUG qi {:04d}] quantiles (x, y, z):\n{}'.format(i, quantiles))
                # else:
                #     print(
                #         '[DEBUG qi {:04d}] min/max:[{:+.6e}, {:+.6e}] finite={} dofs={}'.format(
                #             i, float(qi_flat.min()), float(qi_flat.max()), qi_finite, qi_flat.size
                #         )
                #     )
        ...
        if not require_grad:
            ...
            # Per-frame adjoint clipping. The PD surrogate Jacobian is
            # contractive at fixed-point but non-zero under forward truncation;
            # over many frames with a growing forcing term the adjoint can
            # amplify into inf. Clipping the norm each step keeps the gradient
            # finite without changing it where it is already in range.
            # Opt-out with opt['backward_grad_clip_norm'] = None.
            # Default 1e3 is a conservative cap: initial |dl/dq_next| is O(1)
            # under the normalised random loss in armadillo_env_3d*; adjoint
            # should stay well below this under forward convergence, while
            # still bounding the chain when truncation amplifies residuals.
            grad_clip_norm = backward_opt.get('backward_grad_clip_norm', 1e3) \
                if isinstance(backward_opt, dict) else 1e3

            def _clip_adj(g):
                if grad_clip_norm is None:
                    return g
                g = np.nan_to_num(g, nan=0.0, posinf=grad_clip_norm, neginf=-grad_clip_norm)
                n = np.linalg.norm(g)
                if n > grad_clip_norm:
            ...
            for i in reversed(range(frame_num)):
                ...
                self._deformable.PyBackward(backward_method, q[i], v_clamped[i], sim_act[i], sim_f_ext[i], dt,
                    ...
                    backward_opt_cpp, dl_dq, dl_dv_clamped, dl_da, dl_df, dl_dmat_wi, dl_dact_wi, dl_dstate_pi)
                ...
                for k in range(dofs):
                    if v[i][k] == -velocity_bound or v[i][k] == velocity_bound:
                        dl_dv[k] = 0
                dl_dq_next = _clip_adj(ndarray(dl_dq))
                dl_dv_next = _clip_adj(ndarray(dl_dv))
                if self._stepwise_loss and i != 0:
                    ret = self._stepwise_loss_and_grad(q[i], v[i], i)
                    dqi, dvi = ret[1], ret[2]
                    dl_dq_next += ndarray(dqi)
                    dl_dv_next += ndarray(dvi)
                    dl_dq_next = _clip_adj(dl_dq_next)
                    dl_dv_next = _clip_adj(dl_dv_next)
                dl_act[i]    = _clip_adj(ndarray(dl_da))
                dl_df_ext[i] = _clip_adj(ndarray(dl_df))
            ...
            info['dl_dmat_w'] = np.copy(dl_dmat_w)
