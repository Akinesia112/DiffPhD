...
def export_mp4(folder_name, mp4_name, fps, name_prefix=''):
    ...
    # baseline profile + 8-bit yuv420p is the most compatible H.264 flavour;
    # VS Code, Quicktime, browsers and Android all decode it. ffmpeg 7
    # otherwise defaults to yuv444p10le (high10) from 16-bit PNG input,
    # which most players render as a corrupt red frame.
    os.system('ffmpeg -r ' + str(fps) + ' -i ' + str(tmp_folder / '%08d.png') +
              ' -vcodec libx264 -pix_fmt yuv420p -profile:v baseline -level 3.0 '
              '-movflags +faststart -y ' + str(mp4_name))
...
def render_hex_mesh(hex_mesh, file_name,
    ...
    with open(pbrt_script, 'w') as f:
        ...
        if render_voxel_edge:
            ...
            f.write('Texture "sgrid" "color" "scale" "texture tex1" "grid" "color tex2" [.8 .3 .3]\n')
