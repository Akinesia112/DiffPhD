...
if __name__ == '__main__':
    ...
    opts = ({ 'max_pd_iter': 5000, 'max_ls_iter': 10, 'abs_tol': 1e-9, 'rel_tol': 1e-5, 'verbose': 0, 'thread_ct': thread_ct,
        'use_bfgs': 1, 'use_acc': 1, 'bfgs_history_size': 10},
            { 'max_pd_iter': 5000, 'max_ls_iter': 10, 'abs_tol': 1e-9, 'rel_tol': 1e-5, 'verbose': 0, 'thread_ct': thread_ct,
        'use_bfgs': 1, 'use_acc': 1, 'bfgs_history_size': 10, 'use_mas': 1},
            { 'max_pd_iter': 5000, 'max_ls_iter': 10, 'abs_tol': 1e-9, 'rel_tol': 1e-5, 'verbose': 0, 'thread_ct': thread_ct,
        'use_bfgs': 0, 'use_acc': 1, 'project_newton': 1, 'use_abs': 1, 'aa_window': 5, 'backward_grad_clip_norm': None},)
    ...
    for _ in tqdm(range(random_guess_num)):
        ...
        print(f'Random trial loss: {loss:.3f}')
    ...
    method_display_names = { 'pd_eigen': 'DiffPD', 'pd_eigen_cuda_mas_pcg': 'DiffPD-MAS', 'pd_eigen_alg_phd': 'Ours' }
    for method, opt in zip(methods, opts):
        ...
        def loss_and_grad(x):
            ...
            print('loss: {:12.6f}, |grad|: {:12.6f}, forward time: {:6.3f}s, backward time: {:6.3f}s'.format(
        ...
        # Gradient sanity check — uncomment to verify analytic == numeric (slow: 33 sim calls).
        # from py_diff_pd.common.grad_check import check_gradients
        # check_gradients(loss_and_grad, x_init, eps=1e-6)

        t0 = time.time()
        result = scipy.optimize.minimize(loss_and_grad, np.copy(x_init),
            method='L-BFGS-B', jac=True, bounds=bounds, options={ 'ftol': 1e-4, 'gtol': 1e-6, 'maxiter': 100 })
        t1 = time.time()
        print(result.message)
        print(result.status)
        print_info('Optimization success: {}'.format(result.success))
