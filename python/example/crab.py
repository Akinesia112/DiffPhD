...
from py_diff_pd.env.crab_env import CrabEnv
from py_diff_pd.common.display import export_mp4


def test_crab(verbose):
    """Example using the C++ heterogeneous Young's modulus implementation for Crab."""
    seed = 42
    folder = Path('crab')

    env = CrabEnv(seed, folder, {
        'youngs_modulus': 5e5,
        'init_rotate_angle': 0,
        'state_force_parameters': [0, 0, 0],
        'spp': 4,
        'visualize_boundary_nodes': True,
        'boundary_node_radius': 0.0015,
    })
    ...
    print(f"{'='*70}")
    print(f"\nElement-level Young's Modulus (via C++ heterogeneous materials):")
    ...
    opts = ({ 'max_pd_iter': 5000, 'max_ls_iter': 10, 'abs_tol': 1e-9, 'rel_tol': 1e-1, 'verbose': 0, 'thread_ct': 4,
        'use_bfgs': 1, 'use_acc': 1, 'bfgs_history_size': 10},
            { 'max_pd_iter': 5000, 'max_ls_iter': 10, 'abs_tol': 1e-9, 'rel_tol': 1e-1, 'verbose': 0, 'thread_ct': 4,
    ...
    # Create forcing pattern: upward pulling motion
    f0 = [np.zeros(dofs).reshape((-1, 3)) for _ in range(frame_num)]
    for t in range(frame_num):
        f_up = np.array([0, 0, 1]) * t / frame_num
        for i in env.max_z_nodes():
            f0[t][i] = f_up * 0.5
    ...
    if verbose:
        for method, opt in zip(methods, opts):
            env.simulate(dt, frame_num, method, opt, q0, v0,
