...
if __name__ == '__main__':
    ...
    opt = { 'max_pd_iter': 5000, 'max_ls_iter': 10, 'abs_tol': 1e-9, 'rel_tol': 1e-4,
              'verbose': 0, 'thread_ct': thread_ct, 'use_bfgs': 0, 'use_acc': 1,
    ...
    _, info = env.simulate(dt, frame_num, method, opt, q0, v0, a0, f0, require_grad=False,
        vis_folder=None) # 'initial_condition_render')
    ...
    a0 = [np.zeros(act_dofs) for _ in range(frame_num)]
    f0 = [np.zeros(dofs) for _ in range(frame_num)]
    # env.simulate(dt, frame_num, method, opt, q0, v0, a0, f0, require_grad=False, vis_folder='groundtruth_render')
    # assert False
    # Load results.
    ...
    def simulate(E_opt, nu_opt, vis_folder):
        ...
        loss, _ = env_opt.simulate(dt, frame_num, method, opt, q0, v0, a0, f0, require_grad=False, vis_folder=vis_folder)
        print('Loss: {:.3e}'.format(loss))
        
    simulate(E_init, nu_init, 'init_render')
    simulate(E_final, nu_final, 'final_render')
    ...
    generate_mesh('init_render', 'init_mesh')
    generate_mesh('final_render', 'final_mesh')
