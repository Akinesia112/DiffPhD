...
if __name__ == '__main__':
    ...
    opt = { 'max_pd_iter': 5000, 'max_ls_iter': 10, 'abs_tol': 1e-9, 'rel_tol': 1e-5, 'verbose': 0, 'thread_ct': 8,
    ...
    def simulate(x, vis_folder):
        a = variable_to_act(x)
        loss, info = env.simulate(dt, frame_num, method, opt, q0, v0, [a for _ in range(frame_num)], f0,
            require_grad=False, vis_folder=vis_folder)
        print('loss: {:3f}, forward time: {:.4f}s'.format(loss, info['forward_time']))

    # Initial guess.
    x_init = data[method][0]['x']
    x_final = data[method][-1]['x']
    simulate(x_init, 'init_render')
    simulate(x_final, 'final_render')
    ...
    generate_mesh('init_render', 'init_mesh', x_init)
    generate_mesh('final_render', 'final_mesh', x_final)
