...
if __name__ == '__main__':
    # het = 1 renders the two-stiffness logo, whose two halves mark the stiff and
    # soft regions of the napkin; het = 0 renders the plain logo.
    het = 1

    # Loop over the two resolutions reported in the paper.
    for s in (25, 50):
        seed = 42
        ...
        dt = 2e-3
        frame_num = 125
        texture_img = 'siggraph_logo_{:d}x{:d}{}.png'.format(s, s, '_het' if het else '')
                ...
                for i in range(frame_num + 1):
                    ...
                    if i > 0:
                        ...
                        renderer.add_hex_mesh(mesh, hex_mesh_texture_coords=hex_mesh_texture_coords, transforms=[('s', 1.)],
                            render_voxel_edge=False, color='EEEEEE', texture_img=texture_img)
                    # Draw ground.
