...
struct NeohookeanTriangleProjectionInRange {
    ...
    const std::vector<NHProjectionProblem2D> & problem;
    const std::vector<mcl::optlib::LBFGS<double,2>> & solver;

public:
    NeohookeanTriangleProjectionInRange(std::vector<Mat2x3R> &_res,
                                        const MatX3R &_pos,
                                        ...
                                        const std::vector<NHProjectionProblem2D> & _problem,
...
//
class PDNeohookeanTriangleEnergyParallel : public PDTriangleStretchingEnergy
{

public:
    PDNeohookeanTriangleEnergyParallel(): PDTriangleStretchingEnergy()
    {
        std::cout<<GREEN<<"Elastic Energy Model: Neo-Hookean Triangle Energy (Parallel Ver.)"<<RESET<<std::endl;
    }
    ...
    {
        tbb::parallel_for(tbb::blocked_range<size_t>(0,getElementDimension()), NeohookeanTriangleProjectionInRange(_proj, pos, *_geometry->getTriangles(), coeff, this->_weight, _area, _restMatrix, _problem, _solver));
    ...
    std::vector<NHProjectionProblem2D> _problem;
