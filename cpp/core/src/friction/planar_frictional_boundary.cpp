...
template<int dim>
const Eigen::Matrix<real, dim, 1> PlanarFrictionalBoundary<dim>::GetDistanceGradient(
        const Eigen::Matrix<real, dim, 1>& q) const {
    return normal_;
}

template<int dim>
const Eigen::Matrix<real, dim, 1> PlanarFrictionalBoundary<dim>::GetNormal(
        const Eigen::Matrix<real, dim, 1>& q) const {
    return normal_;
}
...
PlanarFrictionalBoundary<dim>::GetNormalDifferential(
        const Eigen::Matrix<real, dim, 1>& q) const {
    std::vector<Eigen::Matrix<real, dim, 1>> ret(dim);
    for (int k = 0; k < dim; ++k) {
        ret[k] = Eigen::Matrix<real, dim, 1>::Zero();
    }
    return ret;
}
