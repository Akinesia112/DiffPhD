...
template<int dim>
const Eigen::Matrix<real, dim, 1> SphericalFrictionalBoundary<dim>::GetNormal(
        const Eigen::Matrix<real, dim, 1>& q) const {
    const Eigen::Matrix<real, dim, 1> r = q - center_;
    const real rho = r.norm();
    CheckError(rho > std::numeric_limits<real>::epsilon(),
        "SphericalFrictionalBoundary::GetNormal singular at sphere center.");
    return r / rho;
}

template<int dim>
const Eigen::Matrix<real, dim, 1> SphericalFrictionalBoundary<dim>::GetDistanceGradient(
        const Eigen::Matrix<real, dim, 1>& q) const {
    return GetNormal(q);
}

template<int dim>
const std::vector<Eigen::Matrix<real, dim, 1>>
SphericalFrictionalBoundary<dim>::GetNormalDifferential(
        const Eigen::Matrix<real, dim, 1>& q) const {
    const Eigen::Matrix<real, dim, 1> r = q - center_;
    const real rho = r.norm();
    CheckError(rho > std::numeric_limits<real>::epsilon(),
        "SphericalFrictionalBoundary::GetNormalDifferential singular at sphere center.");

    const Eigen::Matrix<real, dim, 1> n = r / rho;

    std::vector<Eigen::Matrix<real, dim, 1>> ret(dim);
    for (int k = 0; k < dim; ++k) {
        Eigen::Matrix<real, dim, 1> ek = Eigen::Matrix<real, dim, 1>::Zero();
        ek(k) = 1;
        ret[k] = (ek - n(k) * n) / rho;
    }
    return ret;
