...
void Deformable<vertex_dim, element_dim>::AddStateForce(const std::string& force_type, const std::vector<real>& params) {
    ...
    if (force_type == "gravity") {
        ...
        state_forces_.push_back(force);
    } else if (force_type == "mesh_contact") {
        CheckError(param_size >= 7, "Inconsistent params for MeshContactStateForce.");
        const real contact_radius = params[0];
        const real kn_left = params[1];
        const real kn_right = params[2];
        const real kf = params[3];
        const real mu = params[4];
        const int face_num_a = static_cast<int>(params[5]);
        const int face_num_b = static_cast<int>(params[6]);
        const int face_param_count = 3 * (face_num_a + face_num_b);
        CheckError(param_size == 7 + face_param_count, "Inconsistent params for MeshContactStateForce.");
        std::vector<int> faces_a(3 * face_num_a, 0);
        std::vector<int> faces_b(3 * face_num_b, 0);
        for (int i = 0; i < 3 * face_num_a; ++i) {
            faces_a[i] = static_cast<int>(params[7 + i]);
        }
        for (int i = 0; i < 3 * face_num_b; ++i) {
            faces_b[i] = static_cast<int>(params[7 + 3 * face_num_a + i]);
        }
        auto force = std::make_shared<MeshContactStateForce<vertex_dim>>();
        force->Initialize(contact_radius, kn_left, kn_right, kf, mu, faces_a, faces_b);
        state_forces_.push_back(force);
    } else {
