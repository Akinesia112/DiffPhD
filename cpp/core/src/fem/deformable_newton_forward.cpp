...
// Forward Newton method with support for heterogeneous material properties
// Key improvements for heterogeneous materials:
// - Element-wise stiffness weights applied in NewtonMatrix (line 306)
// - ProjectedNewtonMatrix respects material heterogeneity (line 332)
// This ensures consistency with ProjectiveDynamics heterogeneous implementation
...
void Deformable<vertex_dim, element_dim>::ForwardNewton(const std::string& method,
    ...
    for (int contact_iter = 0; contact_iter < max_contact_iter; ++contact_iter) {
        if (verbose_level > 1) std::cout << "Contact iteration " << contact_iter << std::endl;
        ...
        if (verbose_level > 0) std::cout << "1. Elastic force norm: " << force_sol.norm() << std::endl;
        ...
        real energy_sol = ElasticEnergy(q_sol) + ComputePdEnergy(q_sol, use_precomputed_data) + ActuationEnergy(q_sol, a);
        if (verbose_level > 0) std::cout << "2. Elastic energy: " << energy_sol << std::endl;
        ...
        for (int i = 0; i < max_newton_iter; ++i) {
            ...
            if (verbose_level > 0) std::cout << "2.5 Gradient norm: " << grad_sol.norm() << std::endl;
            ...
            if (verbose_level > 0) std::cout << "newton_direction norm: " << newton_direction.norm() << std::endl;
            ...
            if (verbose_level > 0) std::cout << "3. Elastic Energy: " << ElasticEnergy(q_sol_next) << std::endl;
            ...
            for (int j = 0; j < max_ls_iter; ++j) {
                ...
                if (verbose_level > 0) std::cout << "iter: " << j << ", step_size: " << step_size << std::endl << "4. Elastic Energy: " << ElasticEnergy(q_sol_next) << std::endl;
                obj_next = eval_obj(q_sol_next, energy_next);
                if (verbose_level > 1) std::cout << "Line search iteration: " << j << std::endl;
                if (verbose_level > 1) {
            ...
            if (verbose_level > 0) std::cout << "5. Elastic force norm: " << force_sol.norm() << std::endl;
...
const SparseMatrix Deformable<vertex_dim, element_dim>::NewtonMatrix(const VectorXr& q_sol, const VectorXr& a,
    ...
    // PD energy force differential with heterogeneous material weights
    ...
    // Apply element-wise stiffness weights to PD energy terms for heterogeneous materials
    if (!pd_element_energies_.empty()) {
        const int element_num = mesh_.NumOfElements();
        const int sample_num = GetNumOfSamplesInElement();
        
        // Compute cumulative stiffness weight (similar to PD Forward setup)
        real total_pd_weight = 0;
        for (const auto& pair : pd_element_energies_) {
            const auto& energy = pair.first;
            total_pd_weight += energy->stiffness();
        }
        const real element_weight = total_pd_weight * element_volume_ / sample_num;
        
        // Apply weight to PD force differential (scales the Hessian contribution)
        for (auto& triplet : nonzeros_pd) {
            triplet = Eigen::Triplet<real>(triplet.row(), triplet.col(), triplet.value() * element_weight);
        }
    }
    
    // Actuation force differential
    ...
    // Combine all Hessian contributions
...
const SparseMatrix Deformable<vertex_dim, element_dim>::ProjectedNewtonMatrix(const VectorXr& q_sol, const VectorXr& a,
    ...
    // Compute element-wise stiffness weights for heterogeneous materials
    // This ensures consistency with PD Forward heterogeneous setup
    std::vector<real> element_weights(element_num, 1.0);
    if (!pd_element_energies_.empty()) {
        real total_pd_weight = 0;
        for (const auto& pair : pd_element_energies_) {
            const auto& energy = pair.first;
        ...
        const real base_weight = total_pd_weight * element_volume_ / sample_num;
        std::fill(element_weights.begin(), element_weights.end(), base_weight);
    }

    SparseMatrixElements nonzeros(element_num * element_dim * vertex_dim * element_dim * vertex_dim);
    
    // Get element Hessians (elastic force second derivatives)
    std::vector<MatrixXr> local_hessians = LocalElasticForceDifferential(q_sol);
    CheckError(static_cast<int>(local_hessians.size()) == element_num,
        "ProjectedNewtonMatrix: local_hessians size mismatch. Expected "
        + std::to_string(element_num) + ", got " + std::to_string(local_hessians.size()));

    // Apply heterogeneous material weights and perform projection on each element Hessian
    #pragma omp parallel for
    for (int i = 0; i < element_num; ++i) {
        const Eigen::Matrix<int, element_dim, 1> vi = mesh_.element(i);
        
        // Apply element-wise stiffness weight to the Hessian
        Eigen::Matrix<real, element_dim * vertex_dim, element_dim * vertex_dim> H_i = 
            local_hessians[i] * element_weights[i];

        // Check diagonal dominance for definiteness fix
