...
void Deformable<vertex_dim, element_dim>::SetupProjectiveDynamicsSolver(const std::string& method, const real dt,
    ...
    // Build per-element stiffness so heterogeneous AddPdEnergy assignments are reflected in A.
    real base_w = 0;
    if (material_) {
        // Add elastic stiffness: 10%, as used by the reference implementation of
        ...
        base_w += material_->ComputeAverageStiffness(0.1);
        std::cout << "Add elastic stiffness to PD solver: " << base_w << std::endl;
    }
    if (use_heterogeneous_material_) {
        // Add elastic stiffness for heterogeneous materials
        base_w += ComputeHeterogenousAverageStiffness(0.1);
        std::cout << "Add heterogeneous elastic stiffness to PD solver: " << base_w << std::endl;
    }
    std::vector<real> element_w(element_num, base_w);
    ...
    //     for (const int idx : element_indices) element_w[idx] += wi;
    //     std::cout << "Add PD element energy stiffness to PD solver: " << energy->stiffness() << "\tlen(idx): " << element_indices.size() << std::endl;
    // }
    // For each element and for each sample, AS maps q to the deformation gradient F.
    for (int i = 0; i < element_num; ++i) {
        // const real wi = element_w[i];
        // if (wi == 0) continue;
        real w_e = base_w;
        for (const auto& pair : pd_element_energies_) {
            const auto& energy = pair.first;
            ...
            if (element_indices.find(i) != element_indices.end()) {
                w_e += energy->element_stiffness(i);
            }
        }
        ...
        for (int j = 0; j < sample_num; ++j) {
            ...
            for (const auto& triplet: pd_AtA_nonzeros) {
                ...
                const real val = triplet.value() * w_e * element_volume_ / sample_num;
    ...
    // std::cout << "Number of PD vertex energies: " << pd_vertex_energies_.size() << std::endl;
    for (const auto& pair : pd_vertex_energies_) {
        const auto& energy = pair.first;
        const real stiffness = energy->stiffness();
        std::cout << "Add PD vertex energy stiffness to PD solver: " << stiffness << std::endl;
    ...
    // std::cout << "Number of PD muscle energies: " << pd_muscle_energies_.size() << std::endl;
        ...
        for (const int i : pair.second) {
            ...
            for (int j = 0; j < sample_num; ++j)
                for (const auto& triplet: AtMtMA[j]) {
                    ...
                    // std::cout << "Adding PdMuscleEnergy stiffness to PD solver: element " << i << ", sample " << j
                    //     << ", row " << row << ", col " << col << ", val " << val << std::endl;
                    // Skip dofs that are fixed by dirichlet boundary conditions.
                    if (dirichlet_.find(remap_idx[row]) == dirichlet_.end() &&
                        dirichlet_.find(remap_idx[col]) == dirichlet_.end()) {
                        const int r = remap_idx[row];
...
const VectorXr Deformable<vertex_dim, element_dim>::ProjectiveDynamicsLocalStep(const VectorXr& q_cur, const VectorXr& a_cur,
    ...
    const int element_num = mesh_.NumOfElements();
    // Accumulate element contributions first, then scatter to global rhs.
    // This keeps heterogeneous per-element energy handling explicit and thread-safe.
    std::vector<Eigen::Matrix<real, vertex_dim, 1>> pd_rhs_ints(element_num * element_dim,
    ...
    for (const auto& pair : pd_element_energies_) {
        ...
        for (int i = 0; i < element_num; ++i) {
            ...
            const Eigen::Matrix<int, element_dim, 1> vi = mesh_.element(i);
            const real w = energy->element_stiffness(i) * element_volume_ / sample_num;
            ...
            for (int j = 0; j < sample_num; ++j) {
                ...
                for (int k = 0; k < element_dim; ++k)
                    pd_rhs_ints[i * element_dim + k] += w * AtBp.segment(k * vertex_dim, vertex_dim);
            }
        }
    }
    ...
    for (const auto& pair : pd_muscle_energies_) {
        ...
        for (int ei = 0; ei < element_cnt; ++ei) {
            ...
            for (int j = 0; j < sample_num; ++j) {
                ...
                for (int k = 0; k < element_dim; ++k)
                    pd_rhs_ints[i * element_dim + k] += wi * AtBp.segment(k * vertex_dim, vertex_dim);
            }
        }
        act_idx += element_cnt;
    }
    ...
    for (int i = 0; i < element_num; ++i) {
        ...
        for (int k = 0; k < element_dim; ++k)
            pd_rhs.segment(vertex_dim * vi[k], vertex_dim) += pd_rhs_ints[i * element_dim + k];
    }
...
const VectorXr Deformable<vertex_dim, element_dim>::PdNonlinearSolve(const std::string& method,
    ...
    if (verbose_level > 1 ) std::cout << "elastic force norm: " << ElasticForce(q_sol).norm() << std::endl;
    if (verbose_level > 1 ) std::cout << "pd force norm: " << PdEnergyForce(q_sol, use_precomputed_data).norm() << std::endl;
    if (verbose_level > 1 ) std::cout << "actuation force norm: " << ActuationForce(q_sol, a).norm() << std::endl;
    ...
    if (verbose_level > 1 ) std::cout << "elastic energy: " << ElasticEnergy(q_sol) << std::endl;
    if (verbose_level > 1 ) std::cout << "pd energy: " << ComputePdEnergy(q_sol, use_precomputed_data) << std::endl;
    if (verbose_level > 1 ) std::cout << "actuation energy: " << ActuationEnergy(q_sol, a) << std::endl;
    ...
    for (int i = 0; i < max_pd_iter; ++i) {
        ...
        if (use_bfgs) {
            ...
            if (verbose_level > 1 ) std::cout << "q_sol_next norm: " << q_sol_next.norm() << std::endl;
            ...
            if (verbose_level > 1 ) std::cout << "elastic energy next: " << ElasticEnergy(q_sol_next) << std::endl;
            ...
            for (int j = 0; j < max_ls_iter; ++j) {
                ...
                if (verbose_level > 1) std::cout << "obj_cond: " << obj_cond << ", obj_next: " << obj_next << std::endl;
...
void Deformable<vertex_dim, element_dim>::ForwardProjectiveDynamics(const std::string& method,
    ...
    // Implemented in deformable_projective_dynamics_forward_gpu.cpp. The NCP
    // handles all contact internally via GetDistance, so every boundary node is
    // a candidate and no pre-filtering is needed.
    if (method == "pd_eigen_alg_phd" || method == "pd_eigen_pcg_proj_gpu") {
        ForwardProjectiveDynamicsAlgPhd(method, q, v, a, f_ext, dt, options,
    ...
    for (int contact_iter = 0; contact_iter < max_contact_iter; ++contact_iter) {
        ...
        for (const auto& pair : frictional_boundary_vertex_indices_) {
            ...
            const real dist = frictional_boundary_->GetDistance(node_q, node_idx);
            const auto node_f = ext_forces.segment(node_idx * vertex_dim, vertex_dim);
            const real contact_force = (frictional_boundary_->GetLocalFrame(node_q, node_idx).transpose() * node_f)(vertex_dim - 1);
        ...
        if (good || final_iter || early_terminate) {
            q_next = q_sol;
            v_next = (q_next - q) / h;
            if (!good) PrintWarning("The contact set fails to converge after 10 iterations.");
