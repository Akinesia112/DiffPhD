...
#include "solver/mas_preconditioner.h"
#include "solver/cuda_mas_cg_solver.h"
#include "fem/mas_converter.h"
...
void Deformable<vertex_dim, element_dim>::SetupProjectiveDynamicsLocalStepDifferential(const VectorXr& q_cur, const VectorXr& a_cur,
    ...
    // std::cout << "element_num: " << element_num << ", sample_num: " << sample_num << std::endl;
    // std::cout << "Number of PD element energies: " << pd_element_energies_.size() << std::endl;
    ...
    for (int i = 0; i < element_num; ++i) {
        ...
        for (int j = 0; j < sample_num; ++j) {
            ...
            for (const auto& epair : pd_element_energies_) {
                ...
                if (element_indices.find(i) == element_indices.end()) {
                    ++energy_cnt;
                    continue;
                }
                // NOTE: energy->stiffness() (global/average) is intentional here,
                // NOT element_stiffness(i): per-element stiffness in the
                // local-step differential correction destabilises over many
                // frames for heterogeneous materials (E_max/E_min = 100×) and
                // amplifies the gradient to inf. Only this correction term uses
                // the global estimate — the adjoint operator A^{-1} (precomputed
                // LDLT) stays heterogeneity-aware.
    ...
    int energy_idx = 0;
    int act_idx = 0;
    // std::cout << "pd_muscle_energies_.size(): " << pd_muscle_energies_.size() << std::endl;
...
void Deformable<vertex_dim, element_dim>::BackwardProjectiveDynamics(const std::string& method, const VectorXr& q, const VectorXr& v,
    ...
    // ── alg_phd dispatch (Algorithm 4, Zeng et al. 2025) ─────────────────────
    // Implemented in deformable_projective_dynamics_backward_gpu.cpp.
    if (method == "pd_eigen_alg_phd" || method == "pd_eigen_pcg_proj_gpu") {
        BackwardProjectiveDynamicsAlgPhd(method, q, v, a, f_ext, dt,
                                       q_next, v_next, active_contact_idx,
                                       dl_dq_next, dl_dv_next, options,
                                       dl_dq, dl_dv, dl_da, dl_df_ext,
                                       dl_dmat_w, dl_dact_w, dl_dstate_p);
        return;
    }
    ...
    bool use_mas = false;
    if (options.find("use_mas") != options.end()) {
        use_mas = static_cast<bool>(options.at("use_mas"));
    }
    ...
    if (verbose_level) std::cout << "use_precomputed_data: " << use_precomputed_data << std::endl;
    ...
    // if (material_) {
    if (material_ || use_heterogeneous_material_ || use_mas) {
        // SparseMatrix op;
        // if (options.find("project_newton") != options.end()) {
        ...
        if (EndsWith(method, "cuda_mas_pcg")) {
            if (verbose_level > 0) PrintInfo("Using GPU MAS preconditioner with CG solver.");
            // GPU CG solver with MAS preconditioner.
            // MAS setup runs on CPU (AllocatePrecoditioner + PreparePreconditioner),
            // then data is uploaded to GPU and the entire CG loop runs on GPU.

            // 1. Convert Eigen sparse matrix to MAS block-CSR format (CPU).
            MasBlockCsr block_csr;
            block_csr.BuildFromEigen(op, q_next);

            // 2. Set up MAS preconditioner on CPU.
            SE::SeSchwarzPreconditioner mas;
            mas.m_positions  = block_csr.positions.data();
            mas.m_neighbours = block_csr.neighbours.Ptr();
            mas.AllocatePrecoditioner(block_csr.numVerts, 0, 0);

            std::vector<unsigned int> empty_ef(block_csr.numVerts + 1, 0);
            std::vector<unsigned int> empty_ee(block_csr.numVerts + 1, 0);
            std::vector<unsigned int> empty_vf(block_csr.numVerts + 1, 0);
            mas.PreparePreconditioner(
                block_csr.diagonal.data(),
                block_csr.csrOffDiagonals.data(),
                block_csr.CsrRanges(),
                nullptr, nullptr, nullptr,
                empty_ef.data(), empty_ee.data(), empty_vf.data());

            // 3. Extract MAS data and set up GPU solver.
            std::vector<int> coarseTablesFlat = mas.GetCoarseTablesFlat();
            std::vector<int> levelSizeX       = mas.GetLevelSizeX();
            std::vector<int> levelSizeY       = mas.GetLevelSizeY();

            CudaMasCgSolver cuda_solver;
            cuda_solver.Setup(
                op.rows(),
                op.nonZeros(),
                op.outerIndexPtr(),   // CSC outer = CSR row_ptr for symmetric matrix
                op.innerIndexPtr(),
                op.valuePtr(),
                mas.GetInvSymR(),     mas.GetInvSymRSize(),
                mas.GetGoingNext(),   mas.GetGoingNextSize(),
                mas.GetMapperSortedGetOriginal(), block_csr.numVerts,
                coarseTablesFlat.data(),
                levelSizeX.data(), levelSizeY.data(), mas.GetNumLevel(),
                mas.GetTotalSz(), mas.GetTotalNumberClusters()
            );

            // 4. Solve op * z = dl_dq_next_agg on GPU.
            const real tol = rel_tol + abs_tol / dl_dq_next_agg.norm();
            dl_drhs_intermediate.resize(dofs_);
            const int iters = cuda_solver.Solve(
                dl_dq_next_agg.data(),
                dl_drhs_intermediate.data(),
                static_cast<double>(tol),
                /*max_iter=*/10000,
                /*check_interval=*/50);

            if (verbose_level > 0)
                PrintInfo("CUDA MAS CG converged in " + std::to_string(iters) + " iterations.");
        }
        else if (EndsWith(method, "mas_pcg")) {
            if (verbose_level > 0) PrintInfo("Using CPU MAS preconditioner with CG solver.");
            // CPU MAS preconditioner path for backward pass.

            // 1. Convert Eigen sparse matrix to MAS block-CSR format.
            MasBlockCsr block_csr;
            block_csr.BuildFromEigen(op, q_next);

            // 2. Set up MAS preconditioner.
            SE::SeSchwarzPreconditioner mas;
            mas.m_positions  = block_csr.positions.data();
            mas.m_neighbours = block_csr.neighbours.Ptr();
            mas.AllocatePrecoditioner(block_csr.numVerts, 0, 0);

            // Empty collision counts (DiffPD has no self-collision in backward pass).
            std::vector<unsigned int> empty_ef(block_csr.numVerts + 1, 0);
            std::vector<unsigned int> empty_ee(block_csr.numVerts + 1, 0);
            std::vector<unsigned int> empty_vf(block_csr.numVerts + 1, 0);
            mas.PreparePreconditioner(
            ...
            // 3. Set global pointers for MasPreconditioner::_solve_impl.
            global_mas_preconditioner = &mas;
            global_mas_num_verts = block_csr.numVerts;

            // 4. Solve op * z = dl_dq_next_agg via CG with MAS preconditioner.
            Eigen::ConjugateGradient<SparseMatrix, Eigen::Lower|Eigen::Upper,
                                     Eigen::MasPreconditioner<real>> cg;
            const real tol = rel_tol + abs_tol / dl_dq_next_agg.norm();
            cg.setTolerance(tol);
            cg.compute(op);
            dl_drhs_intermediate = cg.solve(dl_dq_next_agg);
            CheckError(cg.info() == Eigen::Success, "MAS CG solver failed.");

            // 5. Clean up.
            global_mas_preconditioner = nullptr;
        }
        else if (EndsWith(method, "pcg")) {
            ...
            // const VectorXr dl_drhs_intermediate = cholesky.solve(dl_dq_next_agg);
            // CheckError(cholesky.info() == Eigen::Success, "Cholesky solver failed.");
            if (verbose_level > 0) PrintInfo("Using PCG solver with original Cholesky preconditioner.");
            ...
            // Solve op * z = dl_dq_next_agg  (equation (14)),
            // where dl_drhs_intermediate is z.
            cg.compute(op);
            dl_drhs_intermediate = cg.solve(dl_dq_next_agg);
            CheckError(cg.info() == Eigen::Success, "CG solver failed.");
            ClearGlobalDeformable();
        } else {
            if (verbose_level > 0) PrintInfo("Using Newton Cholesky solver.");
            // Cholesky by default.
            Eigen::SimplicialLDLT<SparseMatrix> cholesky;
            cholesky.compute(op);
        ...
        if (verbose_level > 0) PrintInfo("Using dense solver.");
        ...
        for (int i = 0; i < max_pd_iter; ++i) {
            if (verbose_level > 1) PrintInfo("PD iteration: " + std::to_string(i));
            if (use_bfgs) {
                ...
                for (int j = 0; j < max_ls_iter; ++j) {
                    ...
                    if (verbose_level > 1) PrintInfo("Line search iteration: " + std::to_string(j));
                    if (verbose_level > 1) {
