...
void Deformable<vertex_dim, element_dim>::AddPdEnergy(const std::string& energy_type, const std::vector<real>& params,
    ...
    if (energy_type == "planar_collision") {
        ...
        const int element_num = mesh_.NumOfElements();
        CheckError(param_size == 1 || param_size == element_num,
            "Inconsistent param size: expected 1 (homogeneous) or element_num=" +
            std::to_string(element_num) + " (per-element), got " + std::to_string(param_size));
        auto energy = std::make_shared<CorotatedPdElementEnergy<vertex_dim>>();
        if (param_size == 1) {
            energy->Initialize(params[0]);
        } else {
            energy->InitializeElementStiffnesses(params);
        }

        // Treat indices as element indices.
        std::set<int> element_indices;
        if (indices.empty()) {
            // Apply to all elements
            for (int i = 0; i < element_num; ++i) {
                element_indices.insert(i);
            }
        } else {
            for (const int idx : indices) {
                CheckError(0 <= idx && idx < element_num, "Element index out of bound.");
                element_indices.insert(idx);
            }
        }

        pd_element_energies_.push_back(std::make_pair(energy, element_indices));
        ...
        pd_element_energies_.push_back(std::make_pair(energy, element_indices));
    } else if (energy_type == "pd_neohookean") {
        // Accepted param layouts (all pick w_e = 2*mu + lambda internally):
        //   [mu, lambda]                                homogeneous
        //   [mu_1, la_1, mu_2, la_2, ..., mu_n, la_n]   per-element (length = 2*element_num)
        const int element_num = mesh_.NumOfElements();
        CheckError(param_size == 2 || param_size == 2 * element_num,
            "pd_neohookean expects params = [mu, lambda] or per-element "
            "[mu_1, la_1, ..., mu_n, la_n] of length 2*element_num=" +
            std::to_string(2 * element_num) + ", got " + std::to_string(param_size));
        auto energy = std::make_shared<NeoHookeanPdElementEnergy<vertex_dim>>();
        if (param_size == 2) {
            energy->InitializeNeoHookean(params[0], params[1]);
        } else {
            std::vector<real> mu_per(element_num), la_per(element_num);
            for (int i = 0; i < element_num; ++i) {
                mu_per[i] = params[2 * i];
                la_per[i] = params[2 * i + 1];
            }
            energy->InitializeNeoHookeanPerElement(mu_per, la_per);
        }

        std::set<int> element_indices;
        if (indices.empty()) {
            for (int i = 0; i < element_num; ++i) element_indices.insert(i);
        } else {
            for (const int idx : indices) {
                CheckError(0 <= idx && idx < element_num, "Element index out of bound.");
                element_indices.insert(idx);
...
const VectorXr Deformable<vertex_dim, element_dim>::PdEnergyForce(const VectorXr& q, const bool use_precomputed_data) const {
    ...
    // std::cout << "PDEF:pd_vertex_energies_.size() = " << pd_vertex_energies_.size() << std::endl;
    for (const auto& pair : pd_vertex_energies_) {
        const auto& energy = pair.first;
        for (const int idx : pair.second) {
            const Eigen::Matrix<real, vertex_dim, 1> qi = q.segment(vertex_dim * idx, vertex_dim);
    ...
    int energy_cnt = 0;
    // std::cout << "PDEF:pd_element_energies_.size() = " << pd_element_energies_.size() << std::endl;
    for (const auto& pair : pd_element_energies_) {
        const auto& energy = pair.first;
        const auto& element_indices = pair.second;
        #pragma omp parallel for
