...
public:
    ...
    // Per-element heterogeneous stiffness. When empty, element_stiffness(i)
    // falls back to the scalar stiffness_. Length must equal element_num if set.
    void InitializeElementStiffnesses(const std::vector<real>& stiffnesses);

    const real stiffness() const { return stiffness_; }
    const real element_stiffness(const int element_index) const {
        if (element_stiffnesses_.empty()) return stiffness_;
        return element_stiffnesses_[element_index];
    }

    virtual const real EnergyDensity(const Eigen::Matrix<real, dim, dim>& F) const;
    virtual const Eigen::Matrix<real, dim, dim> StressTensor(const Eigen::Matrix<real, dim, dim>& F) const;
    ...
    virtual const Eigen::Matrix<real, dim * dim, dim * dim> StressTensorDifferential(
        const Eigen::Matrix<real, dim, dim>& F) const;

    virtual const real EnergyDensity(const DeformationGradientAuxiliaryData<dim>& F_auxiliary,
        const Eigen::Matrix<real, dim, dim>& projection) const;
    virtual const Eigen::Matrix<real, dim, dim> StressTensor(const DeformationGradientAuxiliaryData<dim>& F_auxiliary,
        const Eigen::Matrix<real, dim, dim>& projection) const;
    virtual const Eigen::Matrix<real, dim, dim> StressTensorDifferential(const DeformationGradientAuxiliaryData<dim>& F_auxiliary,
...
private:
    real stiffness_;
    std::vector<real> element_stiffnesses_;
};
