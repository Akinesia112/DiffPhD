...
public:
    ...
    // Initialize with heterogeneous material parameters (per-element Young's modulus).
    void InitializeHeterogeneous(const std::string& binary_file_name, const real density,
        const std::string& material_type, const VectorXr& element_youngs_moduli, const real poissons_ratio);
    void InitializeHeterogeneous(const Eigen::Matrix<real, vertex_dim, -1>& vertices,
        const Eigen::Matrix<int, element_dim, -1>& faces, const real density,
        const std::string& material_type, const VectorXr& element_youngs_moduli, const real poissons_ratio);
    // Python-friendly version using std::vector
    void InitializeHeterogeneous(const std::string& binary_file_name, const real density,
    ...
    const std::vector<std::pair<std::shared_ptr<PdElementEnergy<vertex_dim>>, std::set<int>>>& pd_element_energies() const {
...
protected:
    ...
    // Algorithm 4 from "Fast But Accurate: A Real-Time Hyperelastic Simulator
    // with Robust Frictional Contact" (Zeng et al. 2025).
    // Dispatched when method == "pd_eigen_alg_phd" or "pd_eigen_pcg_proj_gpu".
    // Implemented in deformable_projective_dynamics_forward_gpu.cpp.
    void ForwardProjectiveDynamicsAlgPhd(const std::string& method,
        const VectorXr& q, const VectorXr& v, const VectorXr& a,
        const VectorXr& f_ext, const real dt,
        const std::map<std::string, real>& options,
        VectorXr& q_next, VectorXr& v_next,
        std::vector<int>& active_contact_idx) const;
    ...
    // Backward pass for alg_phd.
    // Implemented in deformable_projective_dynamics_backward_gpu.cpp.
    void BackwardProjectiveDynamicsAlgPhd(const std::string& method,
        const VectorXr& q, const VectorXr& v, const VectorXr& a,
        const VectorXr& f_ext, const real dt,
        const VectorXr& q_next, const VectorXr& v_next,
        const std::vector<int>& active_contact_idx,
        const VectorXr& dl_dq_next, const VectorXr& dl_dv_next,
        const std::map<std::string, real>& options,
        VectorXr& dl_dq, VectorXr& dl_dv, VectorXr& dl_da,
        VectorXr& dl_df_ext, VectorXr& dl_dmat_w,
        VectorXr& dl_dact_w, VectorXr& dl_dstate_p) const;
...
private:
    ...
    void InitializeAfterMesh(const real density,
        const std::string& material_type, const real youngs_modulus, const real poissons_ratio);
    void InitializeAfterMeshHeterogeneous(const real density,
        const std::string& material_type, const VectorXr& element_youngs_moduli, const real poissons_ratio);
    ...
    // Compute stress tensor for heterogeneous materials.
    const Eigen::Matrix<real, vertex_dim, vertex_dim> ComputeHeterogenousStressTensor(
        const int element_idx, const Eigen::Matrix<real, vertex_dim, vertex_dim>& F) const;
    // Compute stress tensor differential (Hessian) for heterogeneous materials.
    const Eigen::Matrix<real, vertex_dim * vertex_dim, vertex_dim * vertex_dim> ComputeHeterogenousStressTensorHessian(
        const int element_idx, const Eigen::Matrix<real, vertex_dim, vertex_dim>& F) const;
    // Compute average stiffness for heterogeneous materials.
    const real ComputeHeterogenousAverageStiffness(const real singular_value_range) const;
    ...
    // alg_phd backward helpers: backprop through local step w.r.t. a and w.
    // Implemented in deformable_projective_dynamics_backward_gpu.cpp.
    void BackwardProjectiveDynamicsLocalStepDa(
        const VectorXr& q_next, const VectorXr& a,
    ...
    void BackwardProjectiveDynamicsLocalStepDw(
    ...
    void BackwardAlgPhdContactBlock(
        const VectorXr& q,
        const VectorXr& v,
        const VectorXr& a,
        const real dt,
        ...
        const std::vector<int>& active_contact_idx,
        const VectorXr& mu,
        VectorXr& dl_dq_contact,
        VectorXr& dl_dv_contact) const;

    struct AlgPhdReplayCache {
        ...
        Eigen::VectorXd E_diag;
        Eigen::VectorXd delta_n;

        Eigen::MatrixXd W;
        Eigen::MatrixXd M_sys;

        std::vector<Eigen::Matrix<real, vertex_dim, 1>> normals;
        std::vector<Eigen::MatrixXd> STS;
    };

    void ReplayAlgPhdFixedActiveSet(
        const VectorXr& q,
        const VectorXr& v,
        const VectorXr& a,
    ...
    VectorXr ApplyAlgPhd_Ainv(
        const AlgPhdReplayCache& cache,
        const VectorXr& rhs) const;

    Eigen::VectorXd ApplyAlgPhd_J(
        const AlgPhdReplayCache& cache,
        const VectorXr& x) const;

    VectorXr ApplyAlgPhd_JT(
        const AlgPhdReplayCache& cache,
        const Eigen::VectorXd& y) const;

    VectorXr ApplyAlgPhd_RqLambda(
        const AlgPhdReplayCache& cache,
        const Eigen::VectorXd& dlambda) const;

    Eigen::VectorXd ApplyAlgPhd_RlambdaLambda(
        const AlgPhdReplayCache& cache,
        const Eigen::VectorXd& dlambda) const;

    VectorXr ApplyAlgPhd_RqQ(
        const AlgPhdReplayCache& cache,
        const VectorXr& a,
        const std::vector<Eigen::Matrix<real, vertex_dim * element_dim,
                                        vertex_dim * element_dim>>& pd_bwd_elem,
    ...
    Eigen::VectorXd ApplyAlgPhd_RlambdaQ(
        const AlgPhdReplayCache& cache,
        const VectorXr& a,
        const std::vector<Eigen::Matrix<real, vertex_dim * element_dim,
                                        vertex_dim * element_dim>>& pd_bwd_elem,
    ...
    VectorXr ApplyAlgPhd_RlambdaQ_T(
        const AlgPhdReplayCache& cache,
    ...
    void SolveAlgPhdFixedActiveAdjoint(
        const AlgPhdReplayCache& cache,
    ...
    // Heterogeneous material: per-element Young's modulus.
    VectorXr element_youngs_moduli_;
    // Poisson's ratio (shared across all elements).
    real poissons_ratio_;
    // Flag to indicate whether heterogeneous materials are used.
    bool use_heterogeneous_material_;
    ...
    std::vector<std::pair<std::shared_ptr<PdElementEnergy<vertex_dim>>, std::set<int>>> pd_element_energies_;
    ...
    // Lambda warm-start: carry contact multipliers across frames so the NCP
    // iteration starts near the converged solution instead of lambda=0.
    mutable std::vector<int> alg_phd_prev_candidates_;
    mutable Eigen::VectorXd alg_phd_prev_lambda_;
    // Forward tape for AlgPhd backward fast path. Stores per-frame contact replay
    // data so backward can consume it directly instead of recomputing via replay.
    mutable std::vector<AlgPhdReplayCache> alg_phd_forward_tape_;
